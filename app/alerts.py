...
def match_resource(ref):
    ...
    if not ref:
        return None, None
    # 单次查询按优先级匹配：资源ID精确 > 名称精确 > 名称模糊
    res = db.fetch_one(
        "SELECT id FROM resources WHERE resource_id=? OR name=? OR name LIKE ? "
        "ORDER BY CASE WHEN resource_id=? THEN 0 WHEN name=? THEN 1 ELSE 2 END LIMIT 1",
        (ref, ref, f"%{ref}%", ref, ref))
    if not res:
        return None, None
    rid = res["id"]
