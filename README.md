...
- ✅ **健康多指标叠加**：错误率/延迟/QPS 归一化同图对比（单指标↔叠加切换）
- ✅ **故障模式识别**：健康页自动识别单指标异常 / 多指标共振（疑似服务级故障）/ 流量骤变
- ✅ **AI 多模型源**：`LLM_PROVIDERS` 配置多个 LLM（deepseek/月之暗面等），AI 排障自动 failover
- ✅ **一键部署**：`./install.sh`（拉镜像或 `--build` 本地构建 → 健康检查）
- ✅ **多语言 UI**：简 / 繁 / En 切换（顶栏，记忆选择）
